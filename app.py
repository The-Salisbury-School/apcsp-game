...
def die_quantity_input(message):
    try:
        ...
        if quantity > dice_in_play:
            print(
                'There aren\'t even that many dice! Be reasonable! The number of dice in play is', dice_in_play)
...
def who_goes_first():
    ...
    for player in all_players:
        ...
        print(player.name + ' rolls and gets ' +
              str(player.roll_total) + '...')
        # time.sleep(1)
        ...
        if player.roll_total > highest_roll & highest_roll != 0:
            ...
            print('The previous highest roller, ' + highest_roller.name +
                  ', rolls and gets ' + str(highest_roller.roll_total) + '.')
            # time.sleep(.5)

            roll(player, 2)
            player.roll_total = sum(player.dice_cup)
            print('The challenger, ' + player.name +
                  ', rolls and gets ' + str(player.roll_total) + '.')
            # time.sleep(.5)
            ...
            if highest_roller.roll_total > player.roll_total:
                ...
                print('No way! They tied again! I\'m letting ' +
                      highest_roller.name + ' keep first!')
                # time.sleep(.5)
            else:
            ...
            print('There was a tie, so I\'ll make ' +
                  player.name + ' roll again.')
            # time.sleep(1)
            player.dice_cup = []
            roll(player, 2)
            player.roll_total = sum(player.dice_cup)
            ...
            if player.roll_total > highest_roll:
                ...
                print('You won\'t believe this, but they tied again! so I\'m letting ' +
                      player.name + ' have first now.')
                # time.sleep(.5)
                highest_roll = player.roll_total
                highest_roller = player
            else:
                print('Sorry, ' + player.name + '. No dice! haha! ' +
                      highest_roller.name + ' holds on to first!')
                # time.sleep(.5)

    print('\nThat\'s all the rolls! ' +
          highest_roller.name + ' will go first!\n\n')
...
def first_turn(player):
    if player == p1:
        ...
        player.bid_quantity = die_quantity_input(
            'And how many do you think there are?:\n')

    else:
        print('Ok ye bucket of bolts! ' +
              player.name + '! Choose a die value:\n')
        # time.sleep(1)
...
def raise_bid_value(player, current_bid):
    player.bid_value = die_value_input(
        'Choose a die value the same or higher:\n')
...
def next_turn(player, bid):
    ...
    if player == p1:
        choice = input('Ok ' + player.name +
                       '! Will you RAISE or CHALLENGE the last bid?\n1: RAISE\n2: CHALLENGE\n')
        if choice in {'1', 'one', 'One'}:
            print('Very well.. the current bid is ' +
                  str(current_bid[1]) + ' ' + str(current_bid[0]) + '\'s.')
            ...
            print(
                'We have a CHALLENGE! hahaha! That\'s it then, ye bilgerats! Show your dice!!')
            reveal_dice()
        else:
            print(
                'That\'s not a choice ye scallywag! Make a real choice or its to the plank with you!!')
            choice = input(
                'Ok ' + player.name + '! NOW WILL YE *RAISE* or *CHALLENGE* THE LAST BID!?\n1: RAISE\n2: CHALLENGE\n')
            if choice in {'1', 'one', 'One'}:
                print('Very well.. the current bid is ' +
                      str(current_bid[1]) + ' ' + str(current_bid[0]) + '\'s.')
                ...
                print('THAT\'S IT LANDLUBBER!!\n' + player.name +
                      ', YOU are a LIAR and you will spend ETERNITY ON THIS SHIP!!')
                game_over()
        ...
        if choice < 9:
            ...
            print('The bid is in... ' + player.name + ' thinks there be ' +
                  str(player.bid[0]) + ' ' + str(player.bid[1]) + '\'s!')
            current_bid = player.bid
            new_player = next_player(player)
            next_turn(new_player, current_bid)
        else:
...
def game_over():
    ...
    print_slower(
        'Busun: Have fun! hahahahaaha HEY CRAZY PETE! I got a new friend for ye! They want to hear ALL of your stories! hahahahaha')
    ...
    print_slower(
        'Seriously.. he has to stop to breathe at some point, doesn\'t he?')
    ...
    print_slower(
        'He\'s still talking.. how is this possible? What will you do now?')
    input("")
    print_slower(
        'IT DOESN\'NT MATTER WHAT YOU WANT TO DO! YOU\'RE HERE FOR ETERNITY, REMEMBER!?')
    ...
    print_slower(
        'The hatch to the upper deck opens and the Bosun sticks his head in.')
    ...
    if choice == '1':
        ...
        print_slower(
            'The hatch shuts and you go back to scrubbing, Crazy Pete talking in the background.. I guess this is just life now...')
        end_game_2()
    elif choice == '2':
        print_slower(
            'Bosun: HAHAHAHAHA Me heart be warmed to know of your new friendship! HAHAHA WELCOME TO THE FLYING DUTCHMAN!')
        print_slower(
            'The hatch shuts and you go back to scrubbing, Crazy Pete talking in the background.. I guess this is just life now...')
        end_game_2()
    else:
        print_slower(
            'Bosun: I CAN\'NT BELIEVE ME EARS! YOU REALLY HAVEN\'T LEARNED YOUR LESSON YET!? WHY IS IT SO HARD TO CHOOSE 1 OR 2???')
        print_slower(
            'The hatch shuts and you go back to scrubbing, Crazy Pete talking in the background.. I guess this is just life now...')
...
print('\nThe first bid is in!\n' + current_player.name + ' bids ' +
      str(first_bid[1]) + ' ' + str(first_bid[0]) + '\'s.')
# time.sleep(1)
